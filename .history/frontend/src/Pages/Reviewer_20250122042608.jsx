  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/review/assigned/${userId}`
      ); // Folosește endpoint-ul pentru articolele asignate
      ...
      setArticles(articlesData); // Stochează articolele asignate
      setAuthors(authorsMap); // Stochează autorii articolelor
    } catch (e) {
      console.error("Eroare la încărcarea articolelor asignate: ", e.message);
      toast.error("Nu s-au putut încărca articolele asignate!");
    }
  };
