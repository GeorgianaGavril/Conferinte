...
function EditArticle() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await axios.patch(`http://localhost:3001/article/${id}`, formValues); // Trimite modificările la server
      toast.success('Articolul a fost actualizat cu succes!');
      navigate(`/author/${resp.idAuthor}`); // Navighează înapoi la dashboard
    } catch (error) {
