...
function ReviewerDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const myArticles = articlesRes.filter(
        (article) => article.idReviewer1 === userId || article.idReviewer2 === userId
      );

      const authorIds = [
        ...new Set(myArticles.map((article) => article.idAuthor)),
      ];
